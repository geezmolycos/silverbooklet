...
以下成组的命令可用来控制程序的流程，实现判断和循环等结构。命令间可以互相嵌套和穿插。「用途」一栏中描述此命令可以实现的控制结构，而「功能」一栏中描述此命令的定义。
|命令|用途|功能|
|----|----|----|
|`if ... fi`|出栈一个对象，若是真值(非0、非空字符串)，执行其中指令，否则跳过|`if`: 出栈一个对象，若是真值，则不执行任何操作，否则跳转到配对的`fi`之后。`fi`命令不执行任何操作。|
|`wh ... hw`|与`if`搭配起来实现while型循环。用法: `wh [入栈判断条件] if ... hw fi`，条件为假时结束循环|`wh`: 不执行任何操作。`hw`: 无条件跳转到配对的`wh`之后。|
|`for ... rof`|实现一定次数的循环，出栈一个对象，为循环次数，每次循环时，入栈当前循环变量（从0开始）。|`for`: 出栈总循环次数，进行记录，入栈0，记录当前循环次数为0，继续执行循环体。`rof`: 配对for的当前循环次数增加1，若还未达到其总循环次数，则入栈当前循环次数，跳转到配对for之后，否则不执行任何操作。|
